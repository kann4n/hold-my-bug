...
function startBugInsertion(charTrigger: number, timeTrigger: number, cooldown: number) {
    ...
    changeTextDisposable = vscode.workspace.onDidChangeTextDocument((event) => {
        charCount += event.contentChanges.reduce((acc, change) => acc + change.text.length, 0);
...
export function activate(context: vscode.ExtensionContext) {
    ...
    let startDisposable = vscode.commands.registerCommand('hold-my-bug.start', () => {
        startBugInsertion(REGULAR_CHAR_TRIGGER, REGULAR_TIME_TRIGGER, REGULAR_COOLDOWN);
        vscode.window.showInformationMessage('Hold My Bug: Regular mode started! 🪲');
    });

    // Register the "extreme" command
    let extremeDisposable = vscode.commands.registerCommand('hold-my-bug.extreme', () => {
        startBugInsertion(EXTREME_CHAR_TRIGGER, EXTREME_TIME_TRIGGER, EXTREME_COOLDOWN);
        vscode.window.showInformationMessage('Hold My Bug: EXTREME mode activated! 💥');
    });

    // Register the "stop" command
    let stopDisposable = vscode.commands.registerCommand('hold-my-bug.stop', () => {
        stopBugInsertion();
        vscode.window.showInformationMessage('Hold My Bug: Emoji insertion stopped! 🛑');
    });

    // Register other useless commands
    let uselessModeDisposable = vscode.commands.registerCommand('hold-my-bug.useless-mode', () => {
        ...
        editor.edit(editBuilder => {
            const newText = `🤡 ${emojis[Math.floor(Math.random() * emojis.length)]}`;
            editBuilder.insert(new vscode.Position(lineNum, charPos), newText);
        });
    });

    let typoDisposable = vscode.commands.registerCommand('hold-my-bug.introduce-typo', () => {
        const editor = vscode.window.activeTextEditor;
        if (!editor) return;
        const doc = editor.document;
        const totalLines = doc.lineCount;
        ...
        if (line.text.length > 0) {
            ...
            const newChar = String.fromCharCode(originalChar.charCodeAt(0) + (Math.random() > 0.5 ? 1 : -1));
            editor.edit(editBuilder => {
                const range = new vscode.Range(new vscode.Position(lineNum, charPos), new vscode.Position(lineNum, charPos + 1));
                editBuilder.replace(range, newChar);
            }).then(() => {
                vscode.window.showInformationMessage('Hold My Bug: Typo introduced! Have fun debugging 🤯');
            });
        }
    });

    let sarcasticDisposable = vscode.commands.registerCommand('hold-my-bug.sarcastic-comment', () => {
        const editor = vscode.window.activeTextEditor;
        if (!editor) return;
        const doc = editor.document;
        const totalLines = doc.lineCount;
        ...
        editor.edit(editBuilder => {
            editBuilder.insert(new vscode.Position(lineNum, 0), formattedComment + '\n');
        }).then(() => {
            vscode.window.showInformationMessage('Hold My Bug: A sarcastic comment has been added. You\'re welcome. 🤪');
        });
    });

    context.subscriptions.push(startDisposable, extremeDisposable, stopDisposable, uselessModeDisposable, typoDisposable, sarcasticDisposable);
}
...
function insertRandomEmoji(cooldown: number) {
    ...
    if (!editor) return;
    const now = Date.now();
    if (now - lastInsert < cooldown) return;
    lastInsert = now;
    const doc = editor.document;
    const totalLines = doc.lineCount;
    if (totalLines === 0) return;
